...
$bdd->cache('select a.id as id, a.nom as nom, b.nom as domname , a.cir as cir from categorie a, domaine b where a.id_domaine=b.id ', '');
$categories = $bdd->exec();
...
for ($i = 0; $i < count($categories); $i++) {
    if ($categories[$i]['cir'] == 0) {
        $categories[$i]['cir'] = "non";
    } else {
        $categories[$i]['cir'] = "oui";
    }
