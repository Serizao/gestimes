...
function current_semaine($n, $o) {
    ...
    $w = date('W'); // Pour la 7 Ã¨me semaine Ã  rechercher
    $year = date('Y');
    for ($i = 1;$i <= 365;$i++) {
...
function transfere_v($id, $date, $user, $time, $vers, $de) {
    ...
    if ($time <= $result[0][0]['nb']) {
        ...
        echo '<div style="border:solid 2px green;background:lightgreen;color:green;padding:1em;display:inline-block" class="droid"> heure modifiÃ© avec succÃ¨s</div>';
    } else {
        echo '<div style="border:solid 2px red; background:pink;color:red;padding:1em;display:inline-block" class="droid">le temps est sans doute trop Ã©levÃ©</div>';
    }
}
function month($i) {
...
function add_mouvement($id, $sens, $heure, $url) {
    ...
    $bdd = new bdd(); //check que le dernier mouvement passer n'Ã©tait pas identique
    ...
    if (!$invalid) { //check identique
        if ($sens == "e" and ($mouv == 's' or empty($mouv))) { //check coÃ©rence du mouvement
            arriver($_SESSION['userid'], $heure);
            echo '<div style="border:solid 2px green;background:lightgreen;color:green;padding:1em;display:inline-block" class="droid"> horaires mis Ã  jour avec succÃ¨s' . $url . '</div><meta http-equiv="refresh" content="2; URL=' . $url . '">';
        } elseif ($sens == "s" and $mouv == 'e') {
            partir($_SESSION['userid'], $heure);
            echo '<div style="border:solid 2px green;background:lightgreen;color:green;padding:1em;display:inline-block" class="droid"> horaires mis Ã  jour avec succÃ¨s</div><meta http-equiv="refresh" content="2; URL=' . $url . '">';
        } else {
...
function count_hour($date, $version) {
    ...
    if (!empty($time)) {
        ...
        if ($version == 0) {
            echo 'vous avez ' . $tempspasser['heure'] . 'h' . $tempspasser['minutes'] . 'min' . $tempspasser['second'] . ' Ã  catÃ©goriser pour ce jour';
        }
        if ($version == 1) {
        ...
        if ($version == 0) {
            echo 'il n\'y a pas de temps a catÃ©goriser pour ce jour';
        }
        if ($version == 1) {
            return 0;
        }
...
function cat_hour($date, $cathour, $nb, $url, $comment) {
    ...
    if ($i > 0 and $a >= 0) {
        ...
        echo '<div style="border:solid 2px green;background:lightgreen;color:green;padding:1em;display:inline-block" class="droid"> modification effectuÃ©e avec succÃ¨s</div><meta http-equiv="refresh" content="2; URL=index.php' . $url . '">';
    } else {
        echo '<div style="border:solid 2px red; background:pink;color:red;padding:1em;display:inline-block" class="droid">erreur : le nombre d\'heure entrÃ© est probablement trop grand</div>';
    }
}
...
function datediff($date1, $date2) {
    ...
    $dateBdd = new DateTime($date1); //Date de dÃ©but du type YYYY-MM-DD (date("Y-m-d");
...
function fnl_of_week($annee, $num) {
    ...
    while ($nb_semaine == '01');
    //La premiÃ¨re semaine officielle est celle qui comprend le 4 janvier.
...
function majhs($id) {
    ...
    if (isset($user[0]['begin'])) {
        ...
        for ($a = 1;$a <= 50;$a++) {
            ...
            $nbnonferie = datediff($dfweek[0], $dfweek[1]); //nombre de jour non feriÃ© dans la semaien traitÃ©e
            $hourcontrat = $ui;
            $now = new DateTime($oo);
            $now = $now->format('Ymd');
        ...
        //partie congÃ©
        $conge_mensuel = 25; // toute les entrÃ© seront avec des fraction sur 12 sous entendu ici 25/12
        ...
        $nextM = $next->format('m');
        $nextY = $next->format('Y');
        ///-------------------------------------------------------------->if($nextY.$nextM<$nowY.$nowM){ check si un mois est passer depyuis la de'rniÃ¨re rÃ©actualisation ou alor l'utilisteur n'est pas connu
        if ($nextY . $nextM < $nowY . $nowM or empty($result['maj'])) {
            if (isset($result['nb_jour'])) {
            ...
            } else { //si c'est un nouvel employÃ© qui n'a pas encore d'entrÃ© en base pour ces congÃ© on check son contrat pour savoir
                $solde = 0;
                ...
                if ($contratD > 15 and $solde > 0) {
                    $solde = $solde - $conge_mensuel; /// si le solde est supÃ©rieure a 0 et que la personne est arrivÃ© aprÃ¨s le 15 on enlÃ¨ve 2.083 jour
...
function addconge($id_motif, $id_user, $begin, $end, $jbegin, $jend) {
    ...
    if ($nbjc <= $nbjrc or $type == 2) {
        ...
        echo '<div style="border:solid 2px green;background:lightgreen;color:green;padding:1em;display:inline-block" class="droid"> conge ajoutÃ© avec succÃ¨s</div><meta http-equiv="refresh" content="2; URL=index.php">';
    } else {
        echo '<div style="border:solid 2px red; background:pink;color:red;padding:1em;display:inline-block" class="droid">vous n\'avez sans doute pas assez de congÃ© pour faire sa</div>';
    }
}
function delconge($id, $user_id) {
    ...
    if ($result[0][0]['type'] == 1 or $result[0][0]['type'] == 2) {
        ...
        $bdd->tab('DELETE FROM `conge` WHERE id=? and id_user=?', $array);
        echo '<div style="border:solid 2px green;background:lightgreen;color:green;padding:1em;display:inline-block" class="droid"> conge supprimÃ© avec succÃ¨s</div><meta http-equiv="refresh" content="2; URL=index.php">';
    }
}
